...
Run Operator e2e tests
    ...
    ${result}=    Run Process  cd ${OPERATOR_GIT_DIR} && $(go env GOPATH)/bin/gotestsum -f standard-verbose --debug --junitfile-project-name rhods-operator-e2e --junitfile-testsuite-name short --junitfile-testcase-classname short --junitfile ${OUTPUT_DIR}/xunit/operator-e2e.xml -- ./tests/e2e/ -run ^TestOdhOperator -v -timeout\=50m --operator-namespace\=$E2E_TEST_OPERATOR_NAMESPACE --test-components\=false --test-services\=false
    ...    shell=true
    ...    stderr=STDOUT
